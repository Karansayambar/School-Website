...
const GalleryPage = () => {
  ...
  return (
    <div className='md:w-[95%] md:m-8 p-8 bg-yellow-100'>
      <h3 className='text-center py-8 px-4 text-2xl font-semibold'>------ GALLERY 📸 ------</h3>
        ...
        {Object.keys(topPhotos).map((category) => (
          <div key={category} className='mb-2 md:w-[600px]' onClick={() => handleCategoryClick(category)}>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4'>
              {topPhotos[category].map((item, index) => (
                <div key={item.id} className={`relative overflow-hidden rounded-lg shadow-lg ${index === 0 ? 'md:col-span-2 md:row-span-2' : 'col-span-1 row-span-1'}`}>
                  {item.url.endsWith('.mp4') ? (
                    ...
                    <img src={item.url} alt={item.name} className='w-full h-full object-cover transition-transform duration-300 transform hover:scale-105' />
                  )}
    ...
    <div className='fixed top-40 inset-0 bg-black bg-opacity-50 flex justify-center items-center z-20'>
      <div className='bg-white rounded-lg p-6 max-w-4xl w-full overflow-y-auto'>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 snap-y h-[700px]'>
          {gallery[category].map((item) => (
            <div key={item.id} className='relative overflow-hidden rounded-lg shadow-lg snap-center'>
              {item.url.endsWith('.mp4') ? (
                <video className='w-full h-full object-cover' controls>
                  <source src={item.url} type="video/mp4" />
                  Your browser does not support the video tag.
